...
router.get('/:id', async (req, res) => {
    ...
    const { rows } = await pool.query('SELECT id, login, role FROM users WHERE id = $1', [id]) // Utilisation de la valeur de l'ID dans la requête SQL
    if (rows.length === 0) {
        return res.status(404).json({ error: 'Utilisateur non trouvé' }) // Gestion du cas où l'utilisateur n'est pas trouvé
    }
    res.json(rows[0]) // Renvoi de l'utilisateur trouvé
})
